...
import { LANGUAGES, translateDom, t, pricingLabel, setCurrentLang, tt, alertT, confirmT } from './i18n.js';
...
import { Menu, X, LogOut, Plus, Edit2, Trash2, Home, Calendar, CalendarDays, CreditCard, Building2, Settings, UserCheck, DoorOpen, FileText, BarChart3, LayoutGrid, Download, Eye } from 'lucide-react';
...
const USER_ROLES = {
  admin: {
    ...
    tabs: ['dashboard','calendar','bookings','checkin','rooms','payments','reports','settings'],
    canCreate: true,
    canEdit: true,
    canDelete: true,
  ...
  accounting: {
    ...
    tabs: ['dashboard','calendar','bookings','payments','reports','settings'],
    canCreate: true,
    canEdit: true,
    canDelete: false,
    canManageSettings: false,
  ...
  housekeeping: {
    ...
    tabs: ['dashboard','calendar','checkin','rooms','settings'],
    canCreate: false,
    canEdit: true,
    canDelete: false,
    canManageSettings: false,
  ...
  viewer: {
    ...
    tabs: ['dashboard','calendar','bookings','settings'],
    canCreate: false,
    canEdit: false,
    canDelete: false,
    canManageSettings: false,
...
export default function UbytovnaApp() {
  ...
  }, [lang, activeTab, loading]); // v6.4.9: prekladame len pri zmene jazyka/tabu/po nacitani - nie pri kazdej zmene dat (vykon)
  ...
  async function fetchData() {
    ...
    try {
      // v6.4.9: workflow sync bezi na pozadi (neblokuje nacitanie) - zrychli kazde nacitanie/obnovu.
      // Stats aj availability sa pocitaju nazivo, takze data su spravne aj bez cakania na sync.
      api('/api/workflow/sync', { method: 'POST' }).catch(() => null);
...
function Sidebar({ open, active, setActive, onLogout, property, lang, role }) {
  const allItems=[
    ...
    ['calendar','Kalendár',CalendarDays],
    ['checkin','Check-in / Check-out',DoorOpen],
    ['rooms','Izby',LayoutGrid],
  ...
  return <aside className={`${open?'w-72':'w-20'} stayhub-sidebar v5-sidebar text-white transition-all flex flex-col shadow-2xl`}>
    <div className="sidebar-head">
      ...
      {open&&<div className="mt-5 space-y-3">
        <div className="sidebar-info-card">
          <div className="sidebar-kicker">{'Aktívny objekt'}</div>
          <div className="sidebar-card-title">🏢 {property?.name || 'Hlavný objekt'}</div>
          <div className="sidebar-card-sub">{property?.city || 'Multi-property'}</div>
        </div>

        <div className="sidebar-role-card">
          <div className="sidebar-kicker">{'Rola'}</div>
          <div className="sidebar-role-text">{roleLabel}</div>
        </div>
      </div>}
    </div>
    ...
    <nav className="flex-1 px-3 py-5 space-y-1 overflow-y-auto sidebar-nav">
      {items.map(([id,label,Icon])=>{
        ...
        return <button key={id} onClick={()=>setActive(id)} className={`sidebar-nav-item ${isActive?'is-active':''}`} title={label}>
    ...
    <div className="p-4 border-t border-white/10">
      <button onClick={onLogout} className="sidebar-logout" title={'Odhlásiť'}>
        <LogOut size={20}/>
        {open&&<span>{'Odhlásiť'}</span>}
      </button>
    </div>
  </aside>;
}
...
function Header({ open, setOpen, lang, setLang, properties, propertyId, setPropertyId, role, user }) {
  return <header className="v5-header bg-white/80 backdrop-blur border-b border-slate-200 sticky top-0 z-10"><div className="flex justify-between items-center px-5 md:px-8 py-4 gap-4">
    ...
    <div className="flex items-center gap-3 flex-wrap justify-end">
      <label className="hidden sm:flex items-center gap-2 text-sm font-bold text-slate-600"><span>{'Objekt'}</span><select value={propertyId} onChange={(e)=>setPropertyId(e.target.value)} className="border border-slate-200 rounded-xl px-3 py-2 bg-white text-slate-800 font-semibold">{properties.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
...
function LanguageSwitcher({ lang, setLang }) {
  return <div className="lang-switcher flex items-center gap-1 rounded-full bg-slate-100 p-1 border border-slate-200" title={'Jazyk'}>
    {LANGUAGES.map((item) => (
      <button
        key={item.code}
...
function Dashboard({ stats, bookings, payments, people, rooms, lang='sk', setActive }) {
  ...
  const cap = Number(stats.total_capacity) || (rooms || []).reduce((sum, r) => sum + Number(r.capacity || 0), 0) || roomOcc.reduce((sum, x) => sum + x.occ.capacity, 0);
...
// v6.4.10: jednotny slovensky nazov stavu (v DB su ulozene rozne jazyky - SK/VI/EN).
function paymentStatusLabel(status) {
  if (paymentStatusPaid(status)) return 'Zaplatené';
  const s = String(status || '').toLowerCase();
  if (['po splatnosti','overdue','quá hạn'].includes(s)) return 'Po splatnosti';
  return 'Čaká';
}
...
function Bookings({ bookings, rooms, companies, payments = [], onRefresh, role }) {
  ...
  async function del(b){ if(!confirmT('Vymazať rezerváciu?')) return; await api(`/api/bookings/${b.id}`,{method:'DELETE'}); onRefresh(); }
...
function CheckInModal({ booking, people, onClose, onSaved }) {
  ...
  async function save(){
    try{
      ...
      if(filledRows.length === 0) return setError('Vyber osobu a doplň údaje.');
  ...
  return <Modal title={`Check-in: ${booking.booking_code}`} onClose={onClose} onSave={save} wide>
    <div className="rounded-2xl bg-slate-50 border border-slate-100 p-4">
      ...
      <div className="mt-3 rounded-xl p-3 text-sm font-bold bg-blue-50 text-blue-800 border border-blue-200">Vyber osobu/lôžko → Pas alebo Občiansky preukaz → odfoť/nahraj doklad → vyplň údaje → Potvrdiť check-in. Vyplnené: {filledCount}/{bookingCapacity} · Voľné: {freeCount}</div>
    </div>
      ...
      {current && <div className="rounded-3xl border border-slate-100 bg-white p-5 space-y-5">
        ...
        <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-5">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div><div className="font-black text-slate-900">Doklad totožnosti</div><div className="text-sm text-slate-500">Odfoť doklad mobilom alebo nahraj fotku z počítača. Fotka sa uloží k check-inu; údaje vyplň ručne.</div></div>
          ...
          {(current.document_file_name||current.document_storage_path) && <div className="mt-4 rounded-2xl bg-white border p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            <div className="flex flex-wrap gap-2">{(current.document_preview_url||current.document_file_url) && <a className="btn-muted" href={current.document_preview_url||current.document_file_url} target="_blank" rel="noreferrer">Skontrolovať fotku</a>}</div>
          </div>}
...
function Payments({ payments, bookings, companies, onRefresh, role }) {
  ...
  async function del(p) {
    if (!confirmT('Vymazať platbu?')) return;
    try {
  ...
  return <div className="space-y-6">
    ...
    <div className="mobile-language-panel bg-white rounded-3xl p-5 border border-slate-100 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">{'Jazyk'}</div>
...
function UserInvitations(){
  ...
  async function createManualUser(){
    if(!form.email) return alertT('Vyplň email používateľa.');
    if(!form.email.includes('@')) return alertT('Email nemá správny formát.');
    if(!form.password || form.password.length < 6) return alertT('Heslo musí mať aspoň 6 znakov.');
    if(form.password !== form.password2) return alertT('Heslá sa nezhodujú.');
    setLoading(true); setMsg(null);
    try{
  ...
  async function changePassword(){
    ...
    if(!passwordChange.password || passwordChange.password.length < 6) return alertT('Heslo musí mať aspoň 6 znakov.');
    if(passwordChange.password !== passwordChange.password2) return alertT('Heslá sa nezhodujú.');
    try{
  ...
  async function deleteUser(profile){
    if(!confirmT(`Vymazať používateľa ${profile.email}?`)) return;
  ...
  return <div className="bg-white rounded-xl p-8 space-y-6">
    ...
    <div className="rounded-3xl border border-slate-100 bg-slate-50 p-5 space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Select label="Rola" value={getRole(form.role).label} onChange={v=>sf('role',v)} opts={Object.entries(USER_ROLES).map(([key,value])=>[key,value.label])}/>
...
function FactoryResetAdmin({ onDone }){
  ...
  async function runReset(){
    if(confirmText !== 'RESET') return alertT('Pre potvrdenie napíšte RESET.');
    if(!confirmT('Naozaj chcete vymazať všetky testovacie dáta?')) return;
    setLoading(true); setMsg(null);
    try{
...
function Modal({title,onClose,onSave,children,wide}){return <div className="fixed inset-0 bg-slate-950/45 backdrop-blur-sm z-[130] flex items-center justify-center p-4"><div className={`modal-card bg-white rounded-3xl shadow-2xl w-full ${wide?'max-w-6xl':'max-w-xl'} max-h-[92vh] overflow-auto`}><div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white/95 backdrop-blur z-10"><h2 className="text-2xl font-black tracking-tight text-slate-950">{title}</h2><button onClick={onClose} className="icon-btn"><X/></button></div><div className="p-6 space-y-5">{children}<div className="flex justify-end gap-3 pt-5 border-t border-slate-100"><button onClick={onClose} className="btn-muted">Zrušiť</button><button onClick={onSave} className="btn-save">Uložiť</button></div></div></div></div>}
...
function Banner({children,type}){const arr=Array.isArray(children)?children:[children];const content=arr.every(c=>typeof c==='string'||typeof c==='number')?tt(arr.join('')):children;return <div className={`mx-5 md:mx-8 mt-5 p-4 border rounded-2xl shadow-sm ${type==='error'?'bg-red-50 border-red-200 text-red-700':'bg-emerald-50 border-emerald-200 text-emerald-700'}`}>{content}</div>}
